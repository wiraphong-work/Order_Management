...
test.beforeEach('test login' , async({page})=>{
  ...
  test('Test_06', async ({ page }) => {
    ...
    await expect(page.locator('[id="item_2_title_link"]')).toHaveText('Sauce Labs Onesie');
    await page.selectOption('.product_sort_container' ,'hilo')
    await expect(page.locator('[id="item_5_title_link"]')).toHaveText('Sauce Labs Fleece Jacket');
  });
